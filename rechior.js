...
;(function() {
  var Builder, arrayToHash, compileFile, compileStyl, compileTemplate, dependencyScripts, documentFile, makeScript, readConfig, stripMarkdown;
    ...
    return {
      ...
      build: function(fileData) {
        return build(fileData).then(function(items) {
          var dependencies, dist, distStyle, results, source;
          results = {
            code: [],
            style: []
          };
          ...
          dist = [];
          results.code.each(function(item) {
            return dist.push({
              path: item.name,
              content: item.code,
              type: "blob"
      ...
      testScripts: function(fileData) {
        ...
        return this.build(fileData).then(function(build) {
          ...
          testProgram = Object.keys(distribution).select(function(path) {
            ...
            return distribution[testPath].content;
          }).join("\n");
      ...
      standAlone: function(build, ref) {
        ...
        scriptTag = ref ? makeScript({
          ...
          html: "" + (this.envDeclaration(build)) + "\n" + program
        });
        content.push("</head>\n<body>\n" + scriptTag + "\n</body>\n</html>");
        return {
          html: content.join("\n"),
